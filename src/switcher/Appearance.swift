...
class Appearance {
    ...
    static var backgroundTintBaseColor: NSColor { currentTheme == .dark ? .black : .white }
    /// extra tint layered on the panel background; nil when the user keeps the theme default look or strips it (negative tint)
    static var backgroundTintColor: NSColor? {
        let tint = Preferences.backgroundTint
        guard tint > 0 else { return nil }
        return backgroundTintBaseColor.withAlphaComponent(tint)
    }
